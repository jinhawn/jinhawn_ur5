// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from ur_custom_interfaces:msg/URCommand.idl
// generated code does not contain a copyright notice
#include "ur_custom_interfaces/msg/detail/ur_command__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `x`
// Member `y`
// Member `depth`
#include "rosidl_runtime_c/string_functions.h"

bool
ur_custom_interfaces__msg__URCommand__init(ur_custom_interfaces__msg__URCommand * msg)
{
  if (!msg) {
    return false;
  }
  // x
  if (!rosidl_runtime_c__String__init(&msg->x)) {
    ur_custom_interfaces__msg__URCommand__fini(msg);
    return false;
  }
  // y
  if (!rosidl_runtime_c__String__init(&msg->y)) {
    ur_custom_interfaces__msg__URCommand__fini(msg);
    return false;
  }
  // depth
  if (!rosidl_runtime_c__String__init(&msg->depth)) {
    ur_custom_interfaces__msg__URCommand__fini(msg);
    return false;
  }
  return true;
}

void
ur_custom_interfaces__msg__URCommand__fini(ur_custom_interfaces__msg__URCommand * msg)
{
  if (!msg) {
    return;
  }
  // x
  rosidl_runtime_c__String__fini(&msg->x);
  // y
  rosidl_runtime_c__String__fini(&msg->y);
  // depth
  rosidl_runtime_c__String__fini(&msg->depth);
}

bool
ur_custom_interfaces__msg__URCommand__are_equal(const ur_custom_interfaces__msg__URCommand * lhs, const ur_custom_interfaces__msg__URCommand * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // x
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->x), &(rhs->x)))
  {
    return false;
  }
  // y
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->y), &(rhs->y)))
  {
    return false;
  }
  // depth
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->depth), &(rhs->depth)))
  {
    return false;
  }
  return true;
}

bool
ur_custom_interfaces__msg__URCommand__copy(
  const ur_custom_interfaces__msg__URCommand * input,
  ur_custom_interfaces__msg__URCommand * output)
{
  if (!input || !output) {
    return false;
  }
  // x
  if (!rosidl_runtime_c__String__copy(
      &(input->x), &(output->x)))
  {
    return false;
  }
  // y
  if (!rosidl_runtime_c__String__copy(
      &(input->y), &(output->y)))
  {
    return false;
  }
  // depth
  if (!rosidl_runtime_c__String__copy(
      &(input->depth), &(output->depth)))
  {
    return false;
  }
  return true;
}

ur_custom_interfaces__msg__URCommand *
ur_custom_interfaces__msg__URCommand__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_custom_interfaces__msg__URCommand * msg = (ur_custom_interfaces__msg__URCommand *)allocator.allocate(sizeof(ur_custom_interfaces__msg__URCommand), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_custom_interfaces__msg__URCommand));
  bool success = ur_custom_interfaces__msg__URCommand__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_custom_interfaces__msg__URCommand__destroy(ur_custom_interfaces__msg__URCommand * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_custom_interfaces__msg__URCommand__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_custom_interfaces__msg__URCommand__Sequence__init(ur_custom_interfaces__msg__URCommand__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_custom_interfaces__msg__URCommand * data = NULL;

  if (size) {
    data = (ur_custom_interfaces__msg__URCommand *)allocator.zero_allocate(size, sizeof(ur_custom_interfaces__msg__URCommand), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_custom_interfaces__msg__URCommand__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_custom_interfaces__msg__URCommand__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_custom_interfaces__msg__URCommand__Sequence__fini(ur_custom_interfaces__msg__URCommand__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_custom_interfaces__msg__URCommand__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_custom_interfaces__msg__URCommand__Sequence *
ur_custom_interfaces__msg__URCommand__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_custom_interfaces__msg__URCommand__Sequence * array = (ur_custom_interfaces__msg__URCommand__Sequence *)allocator.allocate(sizeof(ur_custom_interfaces__msg__URCommand__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_custom_interfaces__msg__URCommand__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_custom_interfaces__msg__URCommand__Sequence__destroy(ur_custom_interfaces__msg__URCommand__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_custom_interfaces__msg__URCommand__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_custom_interfaces__msg__URCommand__Sequence__are_equal(const ur_custom_interfaces__msg__URCommand__Sequence * lhs, const ur_custom_interfaces__msg__URCommand__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_custom_interfaces__msg__URCommand__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_custom_interfaces__msg__URCommand__Sequence__copy(
  const ur_custom_interfaces__msg__URCommand__Sequence * input,
  ur_custom_interfaces__msg__URCommand__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_custom_interfaces__msg__URCommand);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_custom_interfaces__msg__URCommand * data =
      (ur_custom_interfaces__msg__URCommand *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_custom_interfaces__msg__URCommand__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_custom_interfaces__msg__URCommand__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_custom_interfaces__msg__URCommand__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
